...
CFComparisonResult _compareFileInfo(const void *fileInfo1, const void *fileInfo2, void *context) {
    CFStringRef name1 = (CFStringRef)CFDictionaryGetValue(((CFMutableDictionaryRef) fileInfo1), CFSTR("name"));
    CFStringRef name2 = (CFStringRef)CFDictionaryGetValue(((CFMutableDictionaryRef) fileInfo2), CFSTR("name"));
...
static CFIndex CFURLEnumeratorPushURL(CFURLEnumeratorRef enumerator, CFURLRef url, CFErrorRef *error) {
    ...
    while ((current = readdir(dir)) != NULL) {
        ...
        CFDictionarySetValue(fileInfo, CFSTR("name"), fileName);
        CFDictionarySetValue(fileInfo, CFSTR("isDir"), current->d_type == DT_DIR ? kCFBooleanTrue : kCFBooleanFalse);
...
CFURLEnumeratorResult CFURLEnumeratorGetNextURL(CFURLEnumeratorRef enumer, CFURLRef *url, CFErrorRef *error) {
    ...
    Boolean isDir = CFBooleanGetValue(CFDictionaryGetValue(fileInfo, CFSTR("isDir")));
    CFURLRef item = NULL;

    if (url != NULL) {
        CFStringRef name = (CFStringRef)CFDictionaryGetValue(fileInfo, CFSTR("name"));
